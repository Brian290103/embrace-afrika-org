...
export default function EventsCarousel({ events, type }: EventsCarouselProps) {
  ...
  return (
    <Carousel
      ...
      className="w-full"
    >
      <div className="py-3 flex relative items-center gap-3 md:justify-end">
      ...
      <CarouselContent>
        {filteredEvents.map((event) => (
          <CarouselItem
            key={event.id}
            className={`sm:basis-1/2 ${type == "upcoming" ? "xl:basis-1/3" : "lg:basis-1/3"} `}
          >
            <motion.div
            ...
            >
              <div className="w-full flex flex-col gap-3">
                ...
                <div className={"p-3 flex gap-1 bg-muted rounded-xl flex-col"}>
                  ...
                  <p className="text-muted-foreground">{event.caption}</p>
                  <div className="w-full flex items-center pt-3 justify-end">
                    <Link href={`/events/${event.slug}`}>
                      ...
                      >
                        {type == "past" ? "Read More" : "Register Now"}
                        <ArrowRight />
                      </Button>
                    </Link>
                  </div>
